...
#define xfn(x_fn) [](auto& x){ return x_fn; }
...
namespace aux {
    ...
    T filter(const T& container, auto predicate) {
        return map_filter(container, xfn(x), predicate);
    }
    ...
    auto join(const auto& container, const std::string& delimiter) {
        return join(container, delimiter, xfn(x));
    }

    // formats a container to a string [v0, v1, v2, v3]
    std::string strlist(const auto& container) {
        std::stringstream ss;
        ss << "[" << join(container, ", ") << "]";
        return ss.str();
    }
    ...
    // formats a container to a string [v0, v1, v2, v3]
    std::string strlist(const auto& container, auto fn) {
        std::stringstream ss;
        ss << "[" << join(container, ", ", fn) << "]";
        return ss.str();
    }
    ...
    void map_insert(std::map<K, T>& map, const std::string& key, T value) {
        auto [it, result] = map.insert(std::make_pair(key, value));
        if (!result) {
            std::stringstream ss;
            ss << "fail: key " << key << " already exists";
            throw std::runtime_error(ss.str());
        }
    }

    template <typename K, typename T>
    T map_at(const std::map<K, T>& map, const K& key) {
        auto it = map.find(key);
        if (it == map.end()) {
            std::stringstream ss;
            ss << "fail: key " << key << " not found";
            throw std::runtime_error(ss.str());
        } 
        return it->second;
    }
